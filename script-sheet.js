...
//-----------------------
// appears when the screen-size is small
...
//>> to stop scrolling down the hidden-sidebar!!
...
// Function to show the sidebar when a button or event triggers it
function showSidebar() {
  sideBar.classList.add('show-sidebar');
}

// second dropdown microsoft ai one (appears when the screen-size is small)!!

// first dropdown above news and announcement
var otherDropdowns = document.querySelectorAll(".dropdown-other");

otherDropdowns.forEach(function(dropdown) {
    var selectOther = dropdown.querySelector(".select-other");
    var caretOther = dropdown.querySelector(".caret-other");
    var menuOther = dropdown.querySelector(".menu-other");
    var optionsOther = dropdown.querySelectorAll(".menu-other li");

    selectOther.addEventListener("click", function() { 
        selectOther.classList.toggle("select-clicked-other");
        caretOther.classList.toggle("caret-rotate-other");
        menuOther.classList.toggle("menu-open-other");
    });

    optionsOther.forEach(function(option) {
        option.addEventListener("click", function() {
            selectOther.classList.remove("select-clicked-other");
            caretOther.classList.remove("caret-rotate-other");
            menuOther.classList.remove("menu-open-other");

            optionsOther.forEach(function(option) {
                option.classList.remove("active-other");
            });

            option.classList.add("active-other");
        });
    });
});
...
companyNames.forEach(function(companyName, index) {
    companyName.addEventListener('click', function() {
        // Calculate the position to slide all items together
        var newPosition = -100 * index; // Shift all items based on the clicked company name
...
//  to underline the sticky-navbar!!
